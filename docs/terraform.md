...
| <a name="input_volume_tags"></a> [volume\_tags](#input\_volume\_tags) | Custom volume tags (e.g. `{'EC2Instance': 'XYZ'}`).<br>If defined and `volume_tags_enabled=true` (default) it overrides `tags`. | `map(string)` | `{}` | no |
